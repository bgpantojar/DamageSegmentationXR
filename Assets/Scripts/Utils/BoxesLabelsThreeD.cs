...
namespace DamageSegmentationXR.Utils
{
    public class BoxesLabelsThreeD
    {
        ...
        public (TextMeshPro, List<LineRenderer>) SpawnClassText(List<TextMeshPro> classTextList, TextMeshPro classTextPrefab, LineRenderer lineRendererPrefab, Vector2Int yoloInputImageSize, BoundingBox box, Transform cameraTransform, Vector2 realImageSize, float fv, float cx, float cy, float minSameObjectDistance)
        //public TextMeshPro SpawnClassText(List<TextMeshPro> classTextList, TextMeshPro classTextPrefab, LineRenderer lineRendererPrefab, Vector2Int yoloInputImageSize, BoundingBox box, Transform cameraTransform, Vector2 realImageSize, float fv, float cx, float cy, float minSameObjectDistance)
        {
            ...
            if (!alreadyLabeled)
            {
                ...
                // Spawn bounding box
                List<LineRenderer> lineRenderers = SpawnClassBox(lineRendererPrefab, cameraTransform, box, yoloInputImageSize, realImageSize, fv, cx, cy);
        ...
        public Vector3 getXYThreeD(Transform cameraTransform, float bx, float by, Vector2 yoloInputImageSize, Vector2 realImageSize, float fv, float cx, float cy, bool isBox = false)
        {
            ...
            //Vector3 coordPlanefvat1CameraSpace = new Vector3(xImageNorm, yImageNorm, nfv);
            //Vector3 coordPlanefvat1WorldSpace = cameraTransform.position + nfv * cameraTransform.forward + cameraTransform.right * xImageNorm + cameraTransform.up * yImageNorm;
            ...
            //Debug.Log($"coordPlanefvat1WorldSpace {coordPlanefvat1WorldSpace} rayDirCameraSpace {rayDirCameraSpace}");
            ...
            //if (Physics.Raycast(rayOriginWorldSpace, (coordPlanefvat1WorldSpace - rayOriginWorldSpace), out RaycastHit hitInfo)) // this is to test in play mode. Comment to deploy in hololens
            //if (Physics.Raycast(rayOriginWorldSpace, (coordPlanefvat1WorldSpace - rayOriginWorldSpace), out RaycastHit hitInfo, 10, LayerMask.GetMask("Spatial Mesh"))) // Uncomment to deploy in hololens. With this rays only hit on Spatial Mesh
            //if (Physics.SphereCast(rayOriginWorldSpace, 0.15f, (coordPlanefvat1WorldSpace - rayOriginWorldSpace), out var hitInfo))
            //if (Physics.SphereCast(rayOriginWorldSpace, 0.15f, (coordPlanefvat1WorldSpace - rayOriginWorldSpace), out var hitInfo, 10, LayerMask.GetMask("Spatial Mesh")))
            {
            ...
            */
            ...
            public List<LineRenderer> SpawnClassBox(LineRenderer lineRendererPrefab, Transform cameraTransform, BoundingBox box, Vector2 yoloInputImageSize, Vector2 realImageSize, float fv, float cx, float cy)
        {
            ...
            //Debug.Log($"x {box.x} y {box.y} width {box.width} height {box.height} Corners topLeft {topLeft} topRight {topRight} bottomRight {bottomRight} bottomLeft {bottomLeft}");

            // Get the coordinate of the bounding box corners in 3D space using ray tracing method
            var positionInSpaceTopLeft = getXYThreeD(cameraTransform, topLeft.x, topLeft.y, yoloInputImageSize, realImageSize, fv, cx, cy, true);
            var positionInSpaceTopRight = getXYThreeD(cameraTransform, topRight.x, topRight.y, yoloInputImageSize, realImageSize, fv, cx, cy, true);
            var positionInSpaceBottomRight = getXYThreeD(cameraTransform, bottomRight.x, bottomRight.y, yoloInputImageSize, realImageSize, fv, cx, cy, true);
            var positionInSpaceBottomLeft = getXYThreeD(cameraTransform, bottomLeft.x, bottomLeft.y, yoloInputImageSize, realImageSize, fv, cx, cy, true);
