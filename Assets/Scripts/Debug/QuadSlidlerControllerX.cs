...
public class QuadSliderControllerX : MonoBehaviour
{
    [SerializeField] private TextMeshProUGUI valueDisplayText; // TextMeshPro to display the X position
    ...
    private void Start()
    {
        ...
        // Read the TextMeshPro with the Quad's new X position
        valueDisplayText.text = $"Position X: {quadTransform.position.x:F4}";
    ...
    private void OnSliderValueUpdated(SliderEventData eventData)
    {
        ...
        // Update the Quad's X position
        Vector3 quadPosition = quadTransform.position;
        quadPosition.x += deltaValue;
        quadTransform.position = quadPosition;

        // Update the TextMeshPro with the Quad's new X position
        valueDisplayText.text = $"Position X: {quadTransform.position.x:F4}";
